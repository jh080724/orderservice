...
public class RedisConfig {
    ...
    // Redus 접속에 필요한 설정을 지정한 후 구현체를 빈으로 등록
    @Bean
    @Qualifier("user-redis-factory")
    // Redis에는 16개의 DB를 제공함. 각 DB마다 서로 다른 데이터를 저장할 수 있다.
...
//        configuration.setPassword("비번");

        return new LettuceConnectionFactory(configuration);
    }

    // RedisTemplate은 redis와 상호작용(insert, select)할 때 redis key, value 의 형식을 정의
    @Bean
